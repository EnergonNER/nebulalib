...
import energon.nebulalib.util.NLibUtilities;
...
public class Config {
    ...
    @Nullable
    public static Config CreateConfigWithDefaultPath(String path, String version) {
        File configDir = new File(Loader.instance().getConfigDir(), path);
        return CreateConfig(configDir, version);
    }

    @Nullable
    public static Config CreateConfig(File file, String version) {
        try {
            Files.createDirectories(file.toPath().getParent());
        } catch (IOException e) {
            return null;
        }
        return new Config(file.exists(), version, file);
    }
    ...
    public static Config ReadConfig(File file, String version) {
        return new Config(file.exists(), version, file);
    }
    ...
    public static class Category {
        ...
        public void save(PrintWriter writer) {
            writer.println("");
            writer.println("");
            writer.println(NLibUtilities.repeat(ElementType.DESCRIPTION.getPrefix(), 106));
